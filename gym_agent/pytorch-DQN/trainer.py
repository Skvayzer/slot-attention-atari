...
class Trainer:
    ...
    def train(self, pre_fr=0):
        ...
        for fr in range(pre_fr + 1, self.config.frames + 1):
            ...
            print("state", next_state, np.array(next_state).shape, file=sys.stderr, flush=True)
            ...
            if self.agent.buffer.size() > self.config.batch_size:
                ...
                losses.append(loss)
                wandb.log({'Loss per frame': loss})

            if fr % self.config.print_interval == 0:
                print("frames: %5d, reward: %5f, loss: %4f episode: %4d" % (fr, np.mean(all_rewards[-10:]), loss, ep_num), file=sys.stderr, flush=True)

            if fr % self.config.log_interval == 0:
                wandb.log({'Reward per episode': all_rewards[-1]})
            ...
            if done:
                ...
                wandb.log({'Best 100-episodes average reward': avg_reward})
                ...
                if len(all_rewards) >= 100 and avg_reward >= self.config.win_reward and all_rewards[-1] > self.config.win_reward:
                    ...
                    print('Ran %d episodes best 100-episodes average reward is %3f. Solved after %d trials ✔' % (ep_num, avg_reward, ep_num - 100), file=sys.stderr, flush=True)
                    if self.config.win_break:
                        break

        if not is_win:
            print('Did not solve after %d episodes' % ep_num, file=sys.stderr, flush=True)
