...
class InvariantSlotAttention(nn.Module):
    ...
    def __init__(self, num_slots, dim, iters=3, eps=1e-8, hidden_dim=128, resolution=(128, 128), enc_hidden_size=64):
        ...
        self.abs_grid_flattened = self.abs_grid.reshape(self.abs_grid.shape[1] * self.abs_grid.shape[2], self.abs_grid.shape[-1])
    ...
    def forward(self, inputs, n_s=None, grid=None,  *args, **kwargs):
        ...
        for t in range(1, self.iters + 1):
            ...
            print(f"\n\nATTENTION! sp fl grid: {self.abs_grid_flattened.expand(b, self.abs_grid_flattened.shape[0], self.abs_grid_flattened.shape[1]).shape} ", file=sys.stderr, flush=True)
